...
class HttpFD(FileDownloader):
    def real_download(self, filename, info_dict):
        ...
        def establish_connection():
            ...
            ctx.is_resume = False
            range_end = (
                range_start + ctx.chunk_size - 1 if ctx.chunk_size else None
            )
            if (
                range_end
                and ctx.data_len is not None
                and range_end >= ctx.data_len
            ):
            ...
            try:
                ...
                if has_range:
                    ...
                    if content_range:
                        ...
                        if content_range_m:
                            if range_start == int(content_range_m.group(1)):
                                ...
                                content_len = int_or_none(
                                    content_range_m.group(3)
                                )
                                accept_content_len = (
                    ...
                    ctx.resume_len = 0
                    ctx.open_mode = "wb"
                ctx.data_len = int_or_none(
                    ctx.data.info().get("Content-length", None)
                )
                return

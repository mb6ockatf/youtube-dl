...
class ExternalFD(FileDownloader):
    ...
    def _write_cookies(self):
        ...
        self.ydl.cookiejar.save(
            self._cookies_tempfile, ignore_discard=True, ignore_expires=True
        )
...
class Aria2pFD(ExternalFD):
    ...
    def _call_downloader(self, tmpfilename, info_dict):
        aria2 = self.aria2p.API(
            self.aria2p.Client(host="http://localhost", port=6800, secret="")
        )
...
class FFmpegFD(ExternalFD):
    ...
    def _call_downloader(self, tmpfilename, info_dict):
        ...
        if protocol in ("m3u8", "m3u8_native"):
            if self.params.get("hls_use_mpegts", False) or tmpfilename == "-":
                ...
                if (
                    ...
                    and is_outdated_version(
                        ffpp._versions["ffmpeg"], "3.2", False
                    )
                ) and (
            ...
            args += [
                "-f",
                EXT_TO_OUT_FORMATS.get(info_dict["ext"], info_dict["ext"]),
            ]

        args = [encodeArgument(opt) for opt in args]
        args.append(
            encodeFilename(ffpp._ffmpeg_filename_argument(tmpfilename), True)
        )
        ...
        with compat_subprocess_Popen(
            args, stdin=subprocess.PIPE, env=env
        ) as proc:
            try:
                retval = proc.wait()

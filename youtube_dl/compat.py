...
try:
    ...
    def _add_init_method_arg(cls):
        ...
        def wrapped_init(self, *args, **kwargs):
            ...
            if any(
                callable(x.__dict__.get("get_method"))
                for x in (self.__class__, self)
                if x != cls
            ):
    ...
    class compat_cookies_SimpleCookie(compat_cookies.SimpleCookie):
        def load(self, rawdata):
            ...
            if not isinstance(rawdata, dict):
                if sys.version_info[:2] != (2, 7) or sys.platform.startswith(
                    "java"
                ):
                    # attribute must have value for parsing
                    rawdata, must_have_value = re.subn(
                        r"(?i)(;\s*)(secure|httponly)(\s*(?:;|$))",
                        r"\1\2=\2\3",
                        rawdata,
                    )
                if sys.version_info[0] == 2:
...
if sys.version_info < (2, 7):
    ...
    _xpath_tokenizer_re = re.compile(
        r"""(?x)
        (                                   # (1)
            '[^']*'|"[^"]*"|                # quoted strings, or
            ::|//?|\.\.|\(\)|[/.*:[\]()@=]  # navigation specials
...
try:
    ...
    def compat_shlex_split(s, comments=False, posix=True):
        if isinstance(s, compat_str):
            s = s.encode("utf-8")
        return list(
            map(lambda s: s.decode("utf-8"), shlex.split(s, comments, posix))
        )
...
if sys.version_info >= (3, 0):
    ...
    def compat_setenv(key, value, env=os.environ):
        def encode(v):
            ...
            return (
                v.encode(get_filesystem_encoding())

...
class TVPEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            "http://www.tvp.pl/sess/tvplayer.php?object_id=%s" % video_id,
            video_id,
        )
        ...
        if error:
            raise ExtractorError(
                "%s said: %s" % (self.IE_NAME, clean_html(error)),
                expected=True,
            )
...
class TVPWebsiteIE(InfoExtractor):
    IE_NAME = "tvp:series"
    _VALID_URL = (
        r"https?://vod\.tvp\.pl/website/(?P<display_id>[^,]+),(?P<id>\d+)"
    )

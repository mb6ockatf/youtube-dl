    ...
    def _real_extract(self, url):
        ...
        def call_playback_api(item, query=None):
            return self._call_api(
                path_templ % item, video_id, item, query=query
            )
...
class NRKTVIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:tv|radio)\.nrk(?:super)?\.no/(?:[^/]+/)*%s" % _EPISODE_RE
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        nrk_id = (
            ...
            or self._search_regex(
                r'data-program-id=["\'](%s)' % NRKTVIE._EPISODE_RE,
                webpage,
                "nrk id",
            )
        )
...
class NRKTVSeasonIE(NRKTVSerieBaseIE):
    ...
    def _real_extract(self, url):
        ...
        title = (
            try_get(data, lambda x: x["titles"]["title"], compat_str)
            or display_id
        )

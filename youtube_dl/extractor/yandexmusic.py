...
class YandexMusicBaseIE(InfoExtractor):
    ...
    def _download_json(self, *args, **kwargs):
        response = super(YandexMusicBaseIE, self)._download_json(
            *args, **kwargs
        )
        ...
        if len(tracks) < len(track_ids):
            ...
            for chunk_num in itertools.count(0):
                ...
                missing_tracks = self._call_api(
                    ...
                    url,
                    item_id,
                    "Downloading missing tracks JSON chunk %d"
                    % (chunk_num + 1),
                    {
...
class YandexMusicArtistTracksIE(YandexMusicArtistBaseIE):
    ...
    _VALID_URL = (
        r"%s/artist/(?P<id>\d+)/tracks" % YandexMusicBaseIE._VALID_URL_BASE
    )
...
class YandexMusicArtistAlbumsIE(YandexMusicArtistBaseIE):
    ...
    _VALID_URL = (
        r"%s/artist/(?P<id>\d+)/albums" % YandexMusicBaseIE._VALID_URL_BASE
    )

        ...
        return merge_dicts(
            ...
            traverse_obj(
                ...
                (
                    ...
                    {
                        "title": (
                            ("programme_title", "series_title"),
                            T(txt_or_none),
                        ),
                        "description": ("full_billing", T(txt_or_none)),
                        "duration": (
                            "duration",
                            T(partial_f(float_or_none, invscale=60)),
                        ),
                    },
                ),
                get_all=False,
...
class S4CSeriesIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            (
                self.url_result(
                    "https://www.s4c.cymru/clic/programme/" + episode_id,
                    S4CIE,
                    episode_id,
                )
                for episode_id in traverse_obj(
                    series_details, ("other_progs_in_series", Ellipsis, "id")
                )
            ),
            playlist_id=series_id,
            playlist_title=traverse_obj(
                series_details,
                ("full_prog_details", 0, "series_title", T(txt_or_none)),
            ),
        )

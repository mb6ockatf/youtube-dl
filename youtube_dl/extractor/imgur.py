...
class ImgurBaseIE(InfoExtractor):
    ...
    def _call_api(self, endpoint, video_id, **kwargs):
        return self._download_json(
            ...
            % (endpoint, video_id, self._CLIENT_ID),
            video_id,
            **kwargs
        )
...
class ImgurIE(ImgurBaseIE):
    ...
    def _extract_twitter_formats(self, html, tw_id="twitter", **kwargs):
        ...
        width, height = (
            int_or_none(
                self._html_search_meta(
                    "twitter:player:" + v, html, default=None
                )
            )
    ...
    def _real_extract(self, url):
        ...
        data = self._call_api(
            "media", video_id, fatal=False, expected_status=404
        )
        webpage = (
        ...
        if not traverse_obj(
            ...
            raise ExtractorError(
                "%s is not a video or animated image" % video_id, expected=True
            )
            ...
            (
                ...
                0,
                {
                    ...
                    "acodec": (
                        "metadata",
                        "has_sound",
                        T(lambda b: None if b else "none"),
                    ),
                },
            ),
        )
        ...
        if not any(
            (u == media_url) for u in traverse_obj(formats, (Ellipsis, "url"))
        ):
            formats.append(mung_format(media_fmt))
        tw_url = traverse_obj(tw_formats, (0, "url"))
        if not any(
            (u == tw_url) for u in traverse_obj(formats, (Ellipsis, "url"))
        ):
        ...
        return merge_dicts(
            ...
            ),
            {
                ...
                "title": self._og_search_title(
                    webpage, default="Imgur video " + video_id
                ),
...
class ImgurAlbumIE(ImgurGalleryBaseIE):
    ...
    _TESTS = [
        ...
        # from PR #21693
        {
            ...
            "info_dict": {"id": "iX265HX", "title": "enen-no-shouboutai"},
            "playlist_count": 2,
        },
        {
            "url": "https://imgur.com/a/8pih2Ed",
            "info_dict": {"id": "8pih2Ed"},
            "playlist_mincount": 1,
        },
    ]

...
class GenericIE(InfoExtractor):
    ...
    def _extract_rss(self, url, video_id, doc):
        ...
        playlist_desc = (
            None if playlist_desc_el is None else playlist_desc_el.text
        )
        ...
        for it in doc.findall("./channel/item"):
            ...
            def itunes(key):
                return xpath_text(
                    it,
                    xpath_with_ns("./itunes:%s" % key, NS_MAP),
                    default=None,
                )
        ...
        def getlicensetoken(license):
            ...
            def parts():
                for o in range(0, center + 1):
                    for i in range(1, 5):
                        yield compat_str(
                            (int(license[o + i]) + int(modlicense[o])) % 10
                        )
    ...
    def _real_extract(self, url):
        ...
        if (
            not self._downloader.params.get("test", False)
        ...
        mobj = (
            ...
            or re.search(
                r'data-ooyala-video-id\s*=\s*[\'"](?P<ec>.{32})[\'"]', webpage
            )
        )
        if mobj is not None:
        ...
        if mobj is not None:
            return self.url_result(
                compat_urllib_parse_unquote(mobj.group("url"))
            )
        ...
        if tnaflix_urls:
            return self.playlist_from_matches(
                ...
                video_id,
                video_title,
                ie=TNAFlixNetworkEmbedIE.ie_key(),
            )
        ...
        mobj = re.search(
            ...
            % CondeNastIE.EMBED_URL,
            webpage,
        )
        if mobj is not None:
            return self.url_result(
                self._proto_relative_url(mobj.group("url"), scheme="http:"),
                "CondeNast",
            )
        ...
        if mobj is not None:
            return self.url_result(
                "eagleplatform:%(host)s:%(id)s" % mobj.groupdict(),
                "EaglePlatform",
            )
        ...
        if found:
            ...
            return merge_dicts(
                self._extract_kvs(url, webpage, video_id), info_dict
            )

...
class VikiBaseIE(InfoExtractor):
    ...
    def _api_query(self, path, version=4, **kwargs):
        ...
        return query + "".join(
            "&{name}={val}.format(**locals())" for name, val in kwargs.items()
        )
...
class VikiIE(VikiBaseIE):
    IE_NAME = "viki"
    _VALID_URL = (
        r"%s(?:videos|player)/(?P<id>[0-9]+v)" % VikiBaseIE._VALID_URL_BASE
    )
    _TESTS = [
        {
            ...
            "expected_warnings": [
                "Unknown MIME type image/jpeg in DASH manifest"
            ],
            "params": {
                "format": "bestvideo",
            },
    ...
    def _real_extract(self, url):
        ...
        subtitles = dict(
                ...
                [
                    {
                        ...
                        % self._api_query(
                            "videos/{0}/auth_subtitles/{1}.{2}".format(
                                video_id, lang, ext
                            ),
                            stream_id=stream_id,
                        ),
            ...
            for lang in (video.get("subtitle_completions") or {}).keys()
        )
...
class VikiChannelIE(VikiBaseIE):
    ...
    _VALID_URL = (
        r"%s(?:tv|news|movies|artists)/(?P<id>[0-9]+c)"
        % VikiBaseIE._VALID_URL_BASE
    )
    _TESTS = [
        {
        ...
        for video_type in video_types:
            ...
            while True:
                ...
                res = self._call_api(
                    ...
                    % (video_type.title(), page_num),
                )

                for video_id in res.get("response") or []:
                    yield self.url_result(
                        "https://www.viki.com/videos/" + video_id,
                        VikiIE.ie_key(),
                        video_id,
                    )
                if not res.get("more"):

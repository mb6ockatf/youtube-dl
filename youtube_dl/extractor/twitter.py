...
class TwitterBaseIE(InfoExtractor):
    ...
    def _extract_formats_from_vmap_url(self, vmap_url, video_id):
        ...
        if video_url not in urls:
            formats.extend(
                self._extract_variant_formats({"url": video_url}, video_id)
            )
        return formats
        ...
        try:
            ...
            if isinstance(e.cause, compat_HTTPError) and e.cause.code == 403:
                raise ExtractorError(
                    self._parse_json(e.cause.read().decode(), video_id)[
                        "errors"
                    ][0]["message"],
                    expected=True,
                )
...
class TwitterIE(TwitterBaseIE):
    ...
    def _real_extract(self, url):
        ...
        tags = []
        for hashtag in (
            try_get(status, lambda x: x["entities"]["hashtags"], list) or []
        ):
        ...
        info = {
            ...
            "uploader_url": (
                "https://twitter.com/" + uploader_id if uploader_id else None
            ),
            ...
            info.update(
                {
                    ...
                    "duration": float_or_none(
                        video_info.get("duration_millis"), 1000
                    ),
                }
            )
        ...
        if media and media.get("type") != "photo":
            ...
            if card:
                ...
                def get_binding_value(k):
                    ...
                    return try_get(
                        o, lambda x: x[x["type"].lower() + "_value"]
                    )
                ...
                # poll4choice_video, ...
                    ...
                    for suffix in (
                        ...
                        image = (
                            get_binding_value("player_image" + suffix) or {}
                        )
...
class TwitterBroadcastIE(TwitterBaseIE, PeriscopeBaseIE):
    ...
    def _real_extract(self, url):
        ...
        m3u8_id = compat_parse_qs(
            compat_urllib_parse_urlparse(m3u8_url).query
        ).get("type", [None])[0]

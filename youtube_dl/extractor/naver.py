...
class NaverBaseIE(InfoExtractor):
    ...
    def _extract_video_info(self, video_id, vid, key):
        ...
        for caption in get_list("caption"):
            ...
            sub_dict.setdefault(
                dict_get(caption, ("locale", "language")), []
            ).extend(get_subs(caption_url))
...
class NaverIE(NaverBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not vid or not in_key:
            player_auth = try_get(
                player_info_json, lambda x: x["playerOption"]["auth"]
            )
        ...
        info.update(
            {
                ...
                "timestamp": int_or_none(
                    current_clip.get("firstExposureTime"), 1000
                ),
                "duration": parse_duration(
                    current_clip.get("displayPlayTime")
                ),

...
class GBNewsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        stream_data = self._download_json(
            "%s/api/%s/stream/%s"
            % (meta_url, "show" if dtype == "vod" else dtype, uvid),
            uvid,
            query={
        ...
        stream_url = traverse_obj(
            stream_data, ("response", "stream", T(url_or_none))
        )
        if not stream_url:
        ...
        return {
            ...
            "title": (
                traverse_obj(stream_data, ("title", T(txt_or_none)))
                or self._og_search_title(webpage, default=None)
                or display_id.replace("-", " ").capitalize()
            ),
            "description": self._og_search_description(webpage, default=None),
            "thumbnail": (
                traverse_obj(video_data, ("data-poster", T(url_or_none)))

...
class AnvatoIE(InfoExtractor):
    ...
    def _get_video_json(self, access_key, video_id):
        ...
        video_data_url = self._api_prefix(
            access_key
        ) + "mcp/video/%s?anvack=%s" % (video_id, access_key)
        ...
        auth_secret = intlist_to_bytes(
            aes_encrypt(
                bytes_to_intlist(input_data[:64]),
                bytes_to_intlist(self._AUTH_KEY),
            )
        )

        video_data_url += "&X-Anvato-Adst-Auth=" + base64.b64encode(
            auth_secret
        ).decode("ascii")
    ...
    def _get_anvato_videos(self, access_key, video_id):
        ...
        for published_url in video_data["published_urls"]:
            ...
            a_format = {
                ...
                "format_id": (
                    "-".join(
                        filter(None, ["http", published_url.get("cdn_name")])
                    )
                ).lower(),
            ...
            if media_format == "m3u8" and tbr is not None:
                a_format.update(
                    {
                        "format_id": "-".join(
                            filter(None, ["hls", compat_str(tbr)])
                        ),
                        "ext": "mp4",
                    }
                )

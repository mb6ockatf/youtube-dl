...
class InfoExtractor(object):
    ...
    * optionally re-define the _real_initialize() method.
    ...
    def _parse_xml(
        self, xml_string, video_id, transform_source=None, fatal=True
    ):
        if transform_source:
    ...
    def _parse_json(
        self, json_string, video_id, transform_source=None, fatal=True
    ):
        if transform_source:
    ...
    def get_param(self, name, *args, **kwargs):
        default, args = (
            (args[0], args[1:])
            if len(args) > 0
            else (kwargs.pop("default", None), args)
        )
        if self._downloader:
    ...
    def playlist_from_matches(
        ...
        urls = orderedSet(
            self.url_result(
                self._proto_relative_url(getter(m) if getter else m), ie
            )
            for m in matches
        )
        ...
        for _ in range(2):
            try:
                ...
                if fatal:
                    ...
                    self.report_warning(
                        "{0}: {1}".format(msg, error_to_compat_str(e)),
                        video_id=video_id,
                    )
            return default
        ...
        for e in json_ld:
            if "@context" in e:
                ...
                if (
                    ...
                    and video.get("@type") == "VideoObject"
                ):
    ...
    def _search_nuxt_data(self, webpage, video_id, *args, **kwargs):
        ...
        context_name = (
            args[0]
            if len(args) > 0
            else kwargs.get("context_name", "__NUXT__")
        )
        ...
        if formats:
            formats[:] = filter(
                lambda f: self._is_valid_url(
                    ...
                    video_id,
                    item=(
                        "%s video format" % f.get("format_id")
                        if f.get("format_id")
                        else "video"
                    ),
                ),
    ...
    def _m3u8_meta_format(
        self, m3u8_url, ext=None, preference=None, m3u8_id=None
    ):
        return {
    ...
    def _parse_m3u8_formats(
        ...
        if re.search(
            r'#EXT-X-SESSION-KEY:.*?URI="skd://', m3u8_doc
        ):  # Apple FairPlay
            return []
    ...
    def _parse_smil_formats(
        ...
        srcs = []
        media = smil.findall(
            self._xpath_ns(".//video", namespace)
        ...
        for medium in media:
            ...
            src_url = (
                ...
                else compat_urlparse.urljoin(base, src)
            )
    ...
    def _parse_mpd_formats_and_subtitles(
        ...
        def resolve_base_url(element, parent_base_url=None):
            ...
            b_url = traverse_obj(
                element, (T(lambda e: e.find(_add_ns("BaseURL")).text))
            )
        ...
        def extract_multisegment_info(element, ms_parent_info):
            ...
            def extract_Initialization(source):
                ...
                if initialization is not None:
                    ms_info["initialization_url"] = (
                        initialization.get("sourceURL") or base_url
                    )
                    initialization_url_range = initialization.get("range")
                    if initialization_url_range:
                        ms_info["initialization_url_range"] = (
                            initialization_url_range
                        )
            ...
            if segment_list is not None:
                ...
                segment_urls = traverse_obj(
                    segment_urls_e, (Ellipsis, T(lambda e: e.attrib), "media")
                )
                if segment_urls:
        ...
        for period in mpd_doc.findall(_add_ns("Period")):
            ...
            for adaptation_set in period.findall(_add_ns("AdaptationSet")):
                ...
                for representation in adaptation_set.findall(
                    ...
                    content_type = (
                        representation_attrib.get("contentType")
                        or mime_type.split("/")[0]
                    )
                    ...
                    if content_type not in ("video", "audio", "text"):
                        if mime_type == "image/jpeg":
                            ...
                            self.report_warning(
                                "Unknown MIME type %s in DASH manifest"
                                % mime_type
                            )
                            continue
                    ...
                    if (
                        ...
                        if "s" in representation_ms_info:
                            ...
                            for segment_url, segment_url_range in zip(
                                ...
                                representation_ms_info["segment_urls_range"],
                            ):
                                fragments.append(
                                    {
    ...
    def _parse_ism_formats(self, ism_doc, ism_url, ism_id=None):
        ...
        """
        if (
            ism_doc.get("IsLive") == "TRUE"
            or ism_doc.find("Protection") is not None
        ):
            return []
        ...
        for stream in ism_doc.findall("StreamIndex"):
            ...
            for track in stream.findall("QualityLevel"):
                fourcc = track.get(
                    "FourCC",
                    "AACL" if track.get("AudioTag") == "255" else None,
                )
    ...
    def _extract_jwplayer_data(self, webpage, video_id, *args, **kwargs):
        ...
        transform_source = kwargs.pop("transform_source", None)
        kwfind = (
            compat_kwargs({"transform_source": transform_source})
            if transform_source
            else {}
        )
    ...
    def _yes_playlist(self, playlist_id, video_id, *args, **kwargs):
        ...
        smuggled_data = (
            args[0] if len(args) == 1 else kwargs.get("smuggled_data")
        )
        ...
        self.to_screen(
            "Downloading just the {0}{1} because of --no-playlist".format(
                video_label, video_id
            )
            if noplaylist
            else "Downloading {0}{1} - add --no-playlist to download just the {2}{3}".format(
                playlist_label,
                "" if playlist_id is True else " " + playlist_id,
                video_label,
                video_id,
            )
        )
...
class SearchInfoExtractor(InfoExtractor):
    ...
    def _make_valid_url(cls):
        return (
            r"%s(?P<prefix>|[1-9][0-9]*|all):(?P<query>[\s\S]+)"
            % cls._SEARCH_KEY
        )

...
class DRTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if video_id:
            ...
            item = try_get(
                page,
                (lambda x: x["item"], lambda x: x["entries"][0]["item"]),
                dict,
            )
                        ...
                        formats.append(
                            {
                                ...
                                "vcodec": (
                                    "none" if kind == "AudioResource" else None
                                ),
                                "preference": preference,
                            }
                        )
            subtitles_list = asset.get("SubtitlesList") or asset.get(
                "Subtitleslist"
            )

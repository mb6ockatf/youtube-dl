...
class LineTVIE(InfoExtractor):
    _VALID_URL = (
        r"https?://tv\.line\.me/v/(?P<id>\d+)_[^/]+-(?P<segment>ep\d+-\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "thumbnails": [
                ...
                for thumbnail in video_info.get("thumbnails", {}).get(
                    "list", []
                )
            ],
...
class LineLiveBaseIE(InfoExtractor):
    ...
    def _parse_broadcast_item(self, item):
        ...
        return {
            ...
            "channel_url": (
                "https://live.line.me/channels/" + channel_id
                if channel_id
                else None
            ),
...
class LineLiveIE(LineLiveBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for k, v in (
            broadcast.get(
                ("live" if info["is_live"] else "archived") + "HLSURLs"
            )
            or {}
        ).items():
            if not v:
        ...
        if not formats:
            ...
            if archive_status != "ARCHIVED":
                raise ExtractorError(
                    "this video has been " + archive_status.lower(),
                    expected=True,
                )
        self._sort_formats(formats)

...
class NiconicoIE(InfoExtractor):
    ...
    def _login(self):
        ...
        if urlh is False:
            ...
            if (
                compat_parse_qs(parts.query).get("message", [None])[0]
                == "cant_login"
            ):
                login_ok = False
        if not login_ok:
    ...
    def _get_heartbeat_info(self, info_dict):
        ...
        # m3u8 (encryption)
        if (
            try_get(api_data, lambda x: x["media"]["delivery"]["encryption"])
            is not None
        ):
            protocol = "m3u8"
            encryption = self._parse_json(session_api_data["token"], video_id)[
                "hls_encryption"
            ]
    ...
    def _extract_format_for_quality(
        ...
        resolution = try_get(
            video_quality, lambda x: x["metadata"]["resolution"], dict
        ) or {"height": vdict.get("res")}
        vbr = try_get(video_quality, lambda x: x["metadata"]["bitrate"], float)

        return {
            "url": "%s:%s/%s/%s"
            % (protocol, video_id, video_quality["id"], audio_quality["id"]),
            "format_id": format_id,
            "format_note": "DMC %s"
            % try_get(
                video_quality, lambda x: x["metadata"]["label"], compat_str
            ),
    ...
    def _real_extract(self, url):
        ...
        if get_video_info_xml("error"):
            ...
            if error_code == "DELETED":
                raise ExtractorError(
                    "The video has been deleted.", expected=True
                )
            elif error_code == "NOT_FOUND":
                raise ExtractorError("The video is not found.", expected=True)
            elif error_code == "COMMUNITY":
                self.to_screen(
                    "%s: The video is community members only." % video_id
                )
            else:
                raise ExtractorError(
                    "%s reports error: %s" % (self.IE_NAME, error_code)
                )
        ...
        if not quality_info:
            raise ExtractorError(
                "The video can't be downloaded", expected=True
            )
        ...
        if video_real_url:
            ...
            cookies = self._get_cookies("https://nicovideo.jp").output(
                header="", sep="; path=/; domain=nicovideo.jp;\n"
            )

            self.to_screen(
                "%s: %s" % (video_id, "Checking smile format with ffprobe")
            )
            ...
            extension = (
                get_video_info_xml("movie_type") or "mp4"
                if "mp4" in metadata["format"]["format_name"]
            ...
            timestamp = parse_iso8601(
                get_video_info_web("first_retrieve")
            ) or unified_timestamp(get_video_info_web("postedDateTime"))
            metadata_timestamp = (
                parse_iso8601(
                    try_get(v_stream, lambda x: x["tags"]["creation_time"])
                )
                or timestamp
                if extension != "mp4"
                else 0
            ...
            is_source = (
                timestamp < smile_threshold_timestamp or metadata_timestamp > 0
            )
                ...
                formats.append(
                    {
                        "url": video_real_url,
                        "format_id": (
                            "smile" if not is_economy else "smile_low"
                        ),
                        "format_note": (
                            "SMILEVIDEO source"
                            if not is_economy
                        ...
                        "tbr": int_or_none(
                            metadata["format"].get("bit_rate"), scale=1000
                        ),
                        "vbr": int_or_none(
                            v_stream.get("bit_rate"), scale=1000
                        ),
                        "abr": int_or_none(
                            a_stream.get("bit_rate"), scale=1000
                        ),
        ...
        timestamp = timestamp or try_get(
            api_data, lambda x: parse_iso8601(x["video"]["registeredAt"])
        )
        ...
        channel = try_get(
            api_data,
            (lambda x: x["channel"]["name"], lambda x: x["community"]["name"]),
        )
...
class NiconicoPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def get_page_data(pagenum, pagesize):
            return (
                self._download_json(
                    "http://nvapi.nicovideo.jp/v2/mylists/" + list_id,
                    list_id,
...
class NicovideoSearchBaseIE(InfoExtractor):
        ...
        for page_num in pages:
            ...
            for item in results:
                yield self.url_result(
                    "http://www.nicovideo.jp/watch/%s" % item, "Niconico", item
                )
            if not results:
                break
    ...
    def _get_n_results(self, query, n):
        entries = self._entries(
            self._proto_relative_url("//www.nicovideo.jp/search/%s" % query),
            query,
        )
...
class NicovideoSearchDateIE(NicovideoSearchBaseIE, SearchInfoExtractor):
    ...
    def _entries(self, url, item_id, start_date=None, end_date=None):
        start_date, end_date = (
            ...
            end_date or datetime.datetime.now().date(),
        )
    ...
    def _get_entries_for_date(
        ...
        for entry in iter(
            super(NicovideoSearchDateIE, self)._entries(
                url, item_id, query=query, note=note
            )
        ):
            yield entry
...
class NiconicoUserIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?nicovideo\.jp/user/(?P<id>\d+)/?(?:$|[#?])"
    )
    _TEST = {
    ...
    def _entries(self, list_id):
        ...
        while count < total_count:
            json_parsed = self._download_json(
                ...
                % (" page %d" % page_num if page_num else ""),
            )
            if not page_num:
            ...
            for entry in json_parsed["data"]["items"]:
                count += 1
                yield self.url_result(
                    "https://www.nicovideo.jp/watch/%s" % entry["id"]
                )
            page_num += 1

...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def _generate_sapisidhash_header(self, origin="https://www.youtube.com"):
        ...
        sapisidhash = hashlib.sha1(
            "{0} {1} {2}".format(time_now, self._SAPISID, origin).encode(
                "utf-8"
            )
        ).hexdigest()
    ...
    def _extract_video(self, renderer):
        ...
        description = try_get(
            renderer,
            lambda x: x["descriptionSnippet"]["runs"][0]["text"],
            compat_str,
        )
    ...
    def _extract_thumbnails(data, *path_list, **kw_final_key):
        ...
        return traverse_obj(
            ...
            (
                (
                    tuple(
                        variadic(path) + (final_key, Ellipsis)
                ...
                ),
                {
                    "url": (
                        ...
                        T(
                            lambda u: (
                                update_url(u, query=None)
                                if u and "maxresdefault" in u
                                else u
                            )
                        ),
                    ),
    ...
    def _search_results(self, query, params):
        ...
        for page_num in itertools.count(1):
            ...
            token = try_get(
                slr_contents,
                lambda x: x[-1]["continuationItemRenderer"][
    ...
    def _extract_channel_id(
        self, webpage, videodetails={}, metadata={}, renderers=[]
    ):
        channel_id = None
    ...
    def _extract_author_var(
        self, webpage, var_name, videodetails={}, metadata={}, renderers=[]
    ):
        result = None
        paths = {
...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _signature_cache_id(self, example_sig):
        ...
        return ".".join(
            compat_str(len(part)) for part in example_sig.split(".")
        )
    ...
    def _load_player(self, video_id, player_url, fatal=True, player_id=None):
        ...
        return (
            self._code_cache[player_id]
            if fatal
            else self._code_cache.get(player_id)
        )
    ...
    def _decrypt_nsig(self, n, video_id, player_url):
        ...
        if self.get_param("youtube_print_sig_code"):
            self.to_screen(
                "Extracted nsig function from {0}:\n{1}\n".format(
    ...
    def _extract_n_function_name(self, jscode):
        ...
        return self._search_json(
            r"var\s+{0}\s*=".format(re.escape(func_name)),
            jscode,
            "Initial JS player n function list ({0}.{1})".format(
                func_name, idx
            ),
            func_name,
    ...
    def _mark_watched(self, video_id, player_response):
        playback_url = url_or_none(
            try_get(
                player_response,
                lambda x: x["playbackTracking"]["videostatsPlaybackUrl"][
                    "baseUrl"
                ],
            )
        )
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            webpage_url + "&bpctr=9999999999&has_verified=1",
            video_id,
            fatal=False,
        )
        ...
        if True or not player_response:
            ...
            client_names = traverse_obj(
                self._INNERTUBE_CLIENTS,
                (
                    T(dict.items),
                    lambda _, k_v: not k_v[1].get("REQUIRE_PO_TOKEN"),
                    0,
                ),
            )[:1]
            if "web" not in client_names:
                ...
                player_response = filter_dict(
                    player_response or {},
                    lambda k, _: k
                    not in ("streamingData", "playabilityStatus"),
                )
            ...
            for client in traverse_obj(
                ...
                api_player_response = self._call_api(
                    ...
                    note=join_nonempty(
                        "Downloading",
                        traverse_obj(
                            query, ("context", "client", "clientName")
                        ),
                        "API JSON",
                        delim=" ",
                    ),
        ...
        def is_agegated(playability):
            ...
            reasons = filter(
                None, (playability.get(r) for r in ("status", "reason"))
            )
            AGE_GATE_REASONS = (
                "confirm your age",
                "age-restricted",
        ...
        def get_playability_status(response):
            return (
                try_get(response, lambda x: x["playabilityStatus"], dict) or {}
            )

        playability_status = get_playability_status(player_response)
        if (
            is_agegated(playability_status)
            and int_or_none(
                self._downloader.params.get("age_limit"), default=18
            )
            >= 18
        ):
        ...
        def get_text(x):
            ...
            return "".join(
                [
                    r["text"]
                    for r in runs
                    if isinstance(r.get("text"), compat_str)
                ]
            )
                    ...
                    "url": update_url_query(
                        f["url"],
                        {
                            "range": "{0}-{1}".format(
                                range_start,
                                min(
                                    range_start + CHUNK_SIZE - 1, f["filesize"]
                                ),
                            ),
                        },
                    )
        ...
        for fmt in streaming_formats:
            ...
            quality = traverse_obj(
                ...
                (
                    (
                        ...
                        (
                            "quality",
                            T(lambda q: q if q and q != "tiny" else None),
                        ),
            ...
            language_preference = (
                ...
                if audio_track.get("audioIsDefault")
                    ...
                    if "descriptive"
                    in (
                        traverse_obj(audio_track, ("displayName", T(lower)))
                        or ""
                    )
                    else -1
                )
            ...
            name = traverse_obj(
                fmt, ("qualityLabel", T(txt_or_none))
            ...
            dct.update(
                traverse_obj(
                    fmt,
                    {
                        ...
                        "format_note": (
                            "qualityLabel",
                            T(lambda x: x or quality),
                        ),
                        # for some formats, fps is wrongly returned as 1
                        "fps": (
                            "fps",
                            T(int_or_none),
                            T(lambda f: f if f > 1 else None),
                        ),
                        ...
                        "tbr": (
                            ...
                            T(lambda t: float_or_none(t, 1000)),
                        ),
                        "width": ("width", T(int_or_none)),
            ...
            single_stream = "none" in (
                dct.get(c) for c in ("acodec", "vcodec")
            )
        ...
        def process_manifest_format(
            f, proto, client_name, itag, all_formats=False
        ):
            ...
            f["quality"] = q(
                traverse_obj(
                    ...
                    (
                        "format_id",
                        T(lambda s: itag_qualities[s.split("-")[0]]),
                    ),
                    default=-1,
                )
            )
        ...
        if formats:
            ...
            reason = get_text(pemr.get("reason")) or playability_status.get(
                "reason"
            )
        ...
        for container in (video_details, microformat):
            for thumbnail in (
                try_get(
                    container, lambda x: x["thumbnail"]["thumbnails"], list
                )
                or []
            ):
                ...
                thumbnails.append(
                    {
                        ...
                        "url": update_url(
                            thumbnail_url, query=None, fragment=None
                        ),
        ...
        for f in formats:
            ...
            if try_call(
                lambda x: float(x.pop("_duration_ms")) / duration < 500,
                args=(f,),
            ):
                self.report_warning(
                    "{0}: Some possibly damaged formats will be deprioritized".format(
                        video_id
                    ),
                    only_once=True,
                )
        ...
        info = {
            ...
            "average_rating": float_or_none(
                video_details.get("averageRating")
            ),
            "age_limit": (
                18
                if (
        ...
        if pctr:
            ...
            def process_subtitles():
                ...
                for caption_track in traverse_obj(
                    pctr, ("captionTracks", lambda _, v: v.get("baseUrl"))
                ):
        ...
        if initial_data:
            ...
            if not chapters:
                for engagment_pannel in (
                    ...
                    contents = try_get(
                        engagment_pannel,
                        lambda x: x["engagementPanelSectionListRenderer"][
                            "content"
                    ...
                    for next_num, content in enumerate(contents, start=1):
                        mmlir = (
                            content.get("macroMarkersListItemRenderer") or {}
                        )
            ...
            if not owner_profile_url:
                owner_profile_url = self._yt_urljoin(
                    self._extract_author_var("", "url", renderers=contents)
                )
            ...
            for content in contents:
                ...
                if vpir:
                    ...
                    if stl:
                        ...
                        if (
                            try_get(
                                vpir, lambda x: x["superTitleIcon"]["iconType"]
                            )
                            == "LOCATION_PIN"
                        ):
                            info["location"] = stl
                            ...
                            mobj = re.search(
                                r"([^\xa0\s].*?)[\xa0\s]*S(\d+)[\xa0\s]*[•]?[\xa0\s]*E(\d+)",
                                stl,
                            )
                            if mobj:
                                info.update(
                    ...
                    for tlb in (
                        ...
                        for getter, regex in [
                            ...
                            label = (try_get(tbr, getter, dict) or {}).get(
                                "label"
                            )
                            if label:
                    ...
                    sbr_tooltip = try_get(
                        vpir,
                        lambda x: x["sentimentBar"]["sentimentBarRenderer"][
                            "tooltip"
                        ],
                    )
                    if sbr_tooltip:
                ...
                if vsir:
                    ...
                    for row in rows:
                        if (
                            try_get(
                                row,
                                lambda x: x["metadataRowRenderer"][
                                    "hasDividerLine"
                                ],
                            )
                            is True
        ...
        return merge_dicts(
            info,
            {
                ...
                and self._yt_urljoin("/channel/" + channel_id),
...
class YoutubeTabIE(YoutubeBaseInfoExtractor):
        ...
        if content_type not in (
            ...
            self.report_warning(
                'Unsupported lockup view model content type "{0}"{1}'.format(
                    content_type, bug_reports_message()
                ),
                only_once=True,
            )
            return
    ...
    def _post_thread_entries(self, post_thread_renderer):
        ...
        for run in runs:
            ...
            if video_id == ep_video_id:
                continue
            yield self.url_result(
                ep_url, ie=YoutubeIE.ie_key(), video_id=video_id
            )
    ...
    def _rich_grid_entries(self, contents):
        for content in contents:
            ...
            # playlist
            renderer = (
                traverse_obj(content, "playlistRenderer", expected_type=dict)
                or {}
            )
            title = self._get_text(renderer, "title")
            playlist_id = renderer.get("playlistId")
    ...
    def _extract_continuation(cls, renderer):
        ...
        for content in contents:
            ...
            continuation_ep = try_get(
                content,
                lambda x: x["continuationItemRenderer"][
                    "continuationEndpoint"
                ],
                dict,
            )
    ...
    def _entries(self, tab, item_id, webpage):
        ...
        if slr_renderer:
            ...
            continuation = None
            slr_contents = (
                try_get(slr_renderer, lambda x: x["contents"], list) or []
            )
            for slr_content in slr_contents:
        ...
        for page_num in itertools.count(1):
            ...
            while count <= retries:
                try:
                    ...
                    if isinstance(
                        e.cause, compat_HTTPError
                    ) and e.cause.code in (500, 503):
                        count += 1
                        if count <= retries:
                            continue
                    raise
            ...
            continuation_items = try_get(
                ...
                lambda x: x[0]["appendContinuationItemsAction"][
                    "continuationItems"
                ],
                list,
            )
    ...
    def _extract_selected_tab(tabs):
        for tab in tabs:
            renderer = (
                dict_get(tab, ("tabRenderer", "expandableTabRenderer")) or {}
            )
    ...
    def _extract_uploader(self, metadata, data):
        ...
        uploader["uploader"] = self._extract_author_var(
            "", "name", renderers=renderers
    ...
    def _extract_alert(cls, data):
        ...
        for alert in traverse_obj(
            ...
            alert_text = traverse_obj(
                alert,
                (None, lambda x: x["alertRenderer"]["text"]),
                get_all=False,
            )
            if not alert_text:
                continue
    ...
    def _extract_from_tabs(self, item_id, webpage, data, tabs):
        ...
        if renderer:
            ...
            playlist_id = (
                txt_or_none(renderer.get("externalId")) or playlist_id
            )
        else:
            renderer = (
                traverse_obj(
    ...
    def _real_extract(self, url):
        ...
        video_id = (
            try_get(
                data,
                lambda x: x["currentVideoEndpoint"]["watchEndpoint"][
                    "videoId"
                ],
                compat_str,
            )
...
class YoutubeFavouritesIE(YoutubeBaseInfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?youtube\.com/my_favorites|:ytfav(?:ou?rites)?"
    )
...
class YoutubeSearchIE(SearchInfoExtractor, YoutubeBaseInfoExtractor):
    ...
    def _get_n_results(self, query, n):
        ...
        entries = itertools.islice(
            self._search_results(query, self._SEARCH_PARAMS),
            0,
...
class YoutubeSearchURLIE(YoutubeBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        params = qs.get("sp", ("",))[-1]
        return self.playlist_result(
            self._search_results(query, params), query, query
        )

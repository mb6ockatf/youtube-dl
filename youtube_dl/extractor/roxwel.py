...
class RoxwelIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?roxwel\.com/player/(?P<filename>.+?)(\.|\?|$)"
    )
    ...
    def _real_extract(self, url):
        ...
        url_page_url = (
            "http://roxwel.com/pl_one_time.php?filename=%s&quality=%s"
            % (filename, best_rate)
        )
        ...
        return {
            ...
            "thumbnail": info.get("player_image_url")
            or info.get("image_url_large"),

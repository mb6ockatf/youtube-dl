...
class VLiveIE(VLiveBaseIE):
    IE_NAME = "vlive"
    _VALID_URL = (
        r"https?://(?:(?:www|m)\.)?vlive\.tv/(?:video|embed)/(?P<id>[0-9]+)"
    )
    ...
    def _login(self):
        ...
        def is_logged_in():
            ...
            return (
                try_get(login_info, lambda x: x["message"]["login"], bool)
                or False
            )
    ...
    def _call_api(self, path_template, video_id, fields=None):
        ...
        try:
            return self._download_json(
                "https://www.vlive.tv/globalv-web/vam-web/"
                + path_template % video_id,
                video_id,
    ...
    def _real_extract(self, url):
        ...
        if video_type == "VOD":
            inkey = self._call_api("video/v1.0/vod/%s/inkey", video_id)[
                "inkey"
            ]
            vod_id = video["vodId"]
            return merge_dicts(
            ...
            if status == "ON_AIR":
                stream_url = self._call_api(
                    "old/v3/live/%s/playInfo", video_id
...
class VLivePostIE(VLiveIE):
    ...
    def _real_extract(self, url):
        ...
        video_seq = str_or_none(
            try_get(post, lambda x: x["officialVideo"]["videoSeq"])
        )
        if video_seq:
        ...
        for idx, video in enumerate(post["attachments"]["video"].values()):
            ...
            entry = None
            if upload_type == "SOS":
                download = self._call_api(self._SOS_TMPL, video_id)[
                    "videoUrl"
                ]["download"]
                formats = []

    ...
    def _extract_relinker_info(self, relinker_url, video_id):
        ...
        for platform in ("mon", "flash", "native"):
            ...
            if not geoprotection:
                geoprotection = (
                    xpath_text(relinker, "./geoprotection", default=None)
                    == "Y"
                )

            if not is_live:
                is_live = (
                    xpath_text(relinker, "./is_live", default=None) == "Y"
                )
            if not duration:
                ...
                formats.append(
                    {
                        ...
                        "format_id": (
                            "http-%d" % bitrate if bitrate > 0 else "http"
                        ),
                    }
                )
    ...
    def _real_extract(self, url):
        ...
        if try_get(
            ...
            (
                ...
                lambda x: x["program_info"]["rights_management"]["rights"][
                    "drm"
                ],
            ),
            dict,
...
class RaiPlayPlaylistIE(InfoExtractor):
    _VALID_URL = (
        r"(?P<base>https?://(?:www\.)?raiplay\.it/programmi/(?P<id>[^/?#&]+))"
    )
    _TESTS = [
        {

...
class ZDFIE(ZDFBaseIE):
    ...
    def _extract_entry(self, url, player, content, video_id):
        ...
        def get_ptmd_path(d):
            return d.get("http://zdf.de/rels/streams/ptmd") or d.get(
                "http://zdf.de/rels/streams/ptmd-template", ""
            ).replace("{playerId}", "ngplayer_2_4")

        ptmd_path = get_ptmd_path(
            try_get(t, lambda x: x["streams"]["default"], dict) or {}
        )
        if not ptmd_path:
    ...
    def _extract_mobile(self, video_id):
        ...
        return {
            ...
            or unified_timestamp(
                try_get(
                    video, lambda x: x["meta"]["editorialDate"], compat_str
                )
            ),
            "thumbnails": thumbnails,
...
class ZDFChannelIE(ZDFBaseIE):
    ...
    def suitable(cls, url):
        return (
            ...
            else super(ZDFChannelIE, cls).suitable(url)
        )

    def _og_search_title(self, webpage, fatal=False):
        title = super(ZDFChannelIE, self)._og_search_title(
            webpage, fatal=fatal
        )
        return (
            re.split(r"\s+[-|]\s+ZDF(?:mediathek)?$", title or "")[0] or None
        )
    ...
    def _real_extract(self, url):
        ...
        if self._downloader.params.get("noplaylist", False):
            entry = next(
                (
                    self.url_result(m.group("url"), ie=ZDFIE.ie_key())
                    for m in matches
                ),
                None,
            )

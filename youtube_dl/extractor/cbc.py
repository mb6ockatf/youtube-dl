...
class CBCIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if CBCPlayerIE.suitable(url)
            else super(CBCIE, cls).suitable(url)
        )
    ...
    def _extract_player_init(self, player_init, display_id):
        ...
        return self.url_result(
            "cbcplayer:%s" % media_id, "CBCPlayer", media_id
        )
...
class CBCWatchBaseIE(InfoExtractor):
    ...
    def _signature(self, email, password):
        ...
        resp = self._download_json(
            self._TOKEN_URL, None, headers=headers, query=query
        )
    ...
    def _real_initialize(self):
        if self._valid_device_token():
            return
        device = (
            self._downloader.cache.load("cbcwatch", self._cache_device_key())
            or {}
        )
        self._device_id, self._device_token = device.get("id"), device.get(
            "token"
        )
        if self._valid_device_token():
            return
...
class CBCWatchIE(CBCWatchBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:gem|watch)\.cbc\.ca/(?:[^/]+/)+(?P<id>[0-9a-f-]+)"
    )
    _TESTS = [
        {
            # geo-restricted to Canada, bypassable

...
class UstreamIE(InfoExtractor):
    ...
    def _get_streams(self, url, video_id, app_id_ver):
        ...
        for trial_count in range(3):
            stream_info = self._get_stream_info(
                ...
                app_id_ver,
                extra_note=(
                    " (try %d)" % (trial_count + 1) if trial_count > 0 else ""
                ),
            )
    ...
    def _parse_segmented_mp4(self, dash_stream_info):
        def resolve_dash_template(template, idx, chunk_hash):
            return template.replace("%", compat_str(idx), 1).replace(
                "%", chunk_hash
            )
    ...
    def _real_extract(self, url):
        ...
        while next_url:
            reply = self._download_json(
                ...
                display_id,
                note="Downloading video information (next: %d)"
                % (len(video_ids) + 1),
            )
            video_ids.extend(

...
class IqiyiSDK(object):
    ...
    def mod(self, modulus):
        ...
        self.target = chunks[0] + "".join(
            map(lambda p: compat_str(p % modulus), ip)
        )
    ...
    def split(self, chunksize):
        ...
        for i in range(len(chunks)):
            ip_part = (
                compat_str(ip[i] % modulus_map[chunksize]) if i < 4 else ""
            )
            if chunksize == 8:
    ...
    def split_ip_time_sum(self):
        chunks, ip = self.preprocess(32)
        self.target = (
            compat_str(sum(ip)) + chunks[0] + self.digit_sum(self.timestamp)
        )

    def split_time_ip_sum(self):
        chunks, ip = self.preprocess(32)
        self.target = (
            self.digit_sum(self.timestamp) + chunks[0] + compat_str(sum(ip))
        )

    ...
    def _real_extract(self, url):
        ...
        if sami_path:
            lang = self._search_regex(
                ...
                sami_path,
                "lang",
                default=compat_urlparse.urlparse(url).netloc.rsplit(".", 1)[
                    -1
                ],
            )
            subtitles[lang] = [
...
class ViafreeIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if TVPlayIE.suitable(url)
            else super(ViafreeIE, cls).suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        program = content["_embedded"]["viafreeBlocks"][0]["_embedded"][
            "program"
        ]
        guid = program["guid"]
        meta = content["meta"]

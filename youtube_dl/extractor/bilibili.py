    ...
    def _report_error(self, result):
        if "message" in result:
            raise ExtractorError(
                "%s said: %s" % (self.IE_NAME, result["message"]),
                expected=True,
            )
        elif "code" in result:
            raise ExtractorError(
                "%s returns error %d" % (self.IE_NAME, result["code"]),
                expected=True,
            )
        else:
    ...
    def _real_extract(self, url):
        ...
        info = {
            ...
            "duration": float_or_none(
                video_info.get("timelength"), scale=1000
            ),
        }
...
class BiliBiliBangumiIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            {
                ...
                "url": smuggle_url(
                    episode["webplay_url"], {"no_bangumi_tip": 1}
                ),
                "ie_key": BiliBiliIE.ie_key(),
                "timestamp": parse_iso8601(
                    episode.get("update_time"), delimiter=" "
                ),
        ...
        entries = sorted(
            entries, key=lambda entry: entry.get("episode_number")
        )

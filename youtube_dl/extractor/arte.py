...
class ArteTVIE(ArteTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not vsr:
            error = None
            if (
                try_get(player_info, lambda x: x["custom_msg"]["type"])
                == "error"
            ):
                error = try_get(
            ...
            if not error:
                error = (
                    "Video %s is not available" % player_info.get("VID")
                    or video_id
                )
...
class ArteTVPlaylistIE(ArteTVBaseIE):
    ...
    def _real_extract(self, url):
            ...
            entries.append(
                {
                    ...
                    "thumbnail": url_or_none(
                        try_get(
                            video, lambda x: x["mainImage"]["url"], compat_str
                        )
                    ),
        ...
        description = collection.get("shortDescription") or collection.get(
            "teaserText"
        )
...
class ArteTVCategoryIE(ArteTVBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?arte\.tv/(?P<lang>%s)/videos/(?P<id>[\w-]+(?:/[\w-]+)*)/?\s*$"
        % ArteTVBaseIE._ARTE_LANGUAGES
    )
    _TESTS = [
        {

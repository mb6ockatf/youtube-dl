...
class XimalayaIE(XimalayaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        audio_info_file = "%s://m.ximalaya.com/tracks/%s.json" % (
            scheme,
            audio_id,
        )
        ...
        if not audio_description:
            audio_description_file = (
                "%s://www.ximalaya.com/sounds/%s/rich_intro"
                % (scheme, audio_id)
            )
            audio_description = self._download_webpage(
                audio_description_file,
                audio_id,
                note="Downloading description file %s"
                % audio_description_file,
                errnote="Unable to download descrip file",
                fatal=False,
            )
            audio_description = (
                audio_description.strip() if audio_description else None
            )
        ...
        return {
            ...
            "uploader_url": (
                self._USER_URL_FORMAT % (scheme, audio_uploader_id)
            ...
            "categories": list(
                filter(
                    None,
                    (
                        audio_info.get("category_name"),
                        audio_info.get("category_title"),
                    ),
                )
            ),

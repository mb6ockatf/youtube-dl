...
class OoyalaBaseIE(InfoExtractor):
    ...
    def _extract(
        ...
        content_tree = self._download_json(content_tree_url, video_id)[
            "content_tree"
        ]
        ...
        if not formats and not auth_data.get("authorized"):
            raise ExtractorError(
                "%s said: %s" % (self.IE_NAME, auth_data["message"]),
                expected=True,
            )
        ...
        return {
            ...
            "thumbnail": metadata.get("thumbnail_image")
            or metadata.get("promo_image"),
...
class OoyalaIE(OoyalaBaseIE):
    ...
    def _build_url_result(cls, embed_code):
        return cls.url_result(
            cls._url_for_embed_code(embed_code), ie=cls.ie_key()
        )

...
class SoundcloudIE(InfoExtractor):
    ...
    def _extract_info_dict(self, info, full_title=None, secret_token=None):
        ...
        def add_format(f, protocol, is_preview=False):
            ...
            preview = is_preview or re.search(
                r"/(?:preview|playlist)/0/30/", f["url"]
            )
            if preview:
        ...
        # New API
        transcodings = (
            try_get(info, lambda x: x["media"]["transcodings"], list) or []
        )
    ...
    def _real_extract(self, url):
        ...
        if track_id:
            ...
            full_title = resolve_title = "%s/%s" % mobj.group(
                "uploader", "title"
            )
...
class SoundcloudPlaylistBaseIE(SoundcloudIE):
    def _extract_set(self, playlist, token=None):
        ...
        for track in tracks:
            ...
            entries.append(
                self.url_result(url, SoundcloudIE.ie_key(), track_id)
            )

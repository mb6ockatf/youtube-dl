...
class RICEIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        player_data = self._download_xml(
            ...
            content_id,
            query={
                "temporaryLinkId": xpath_text(
                    encodings, "temporaryLinkId", fatal=True
                ),
                "contentId": content_id,
            },
        )
        ...
        formats = []
        rtsp_path = xpath_text(
            player_data, self._xpath_ns("RtspPath", self._NS)
        )
        if rtsp_path:
            fmt = {
                "url": rtsp_path,
            ...
            formats.append(fmt)
        for source in player_data.findall(
            self._xpath_ns(".//Source", self._NS)
        ):

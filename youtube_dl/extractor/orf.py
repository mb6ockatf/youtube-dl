...
class ORFRadioBase(InfoExtractor):
    ...
    def _get_api_payload(cls, data, expected_id, in_payload=False):
        if expected_id not in traverse_obj(
            data,
            ("payload",)[: 1 if in_payload else 0]
            + (cls._ID_NAMES, T(txt_or_none)),
        ):
            raise ExtractorError(
                "Unexpected API data result", video_id=expected_id
            )
        return data["payload"]
    ...
    def _entries(cls, data, station, item_type=None):
        if item_type in ("upload", "podcast-episode"):
            yield merge_dicts(
                ...
                cls._extract_podcast_upload(data),
                rev=True,
            )
            return
        ...
        for info in traverse_obj(
            data,
            (
                (("streams", Ellipsis), "stream"),
                T(lambda v: v if v["loopStreamId"] else None),
            ),
        ):
...
class ORFRadioIE(ORFRadioBase):
    ...
    _VALID_URL = (
        r"https?://sound\.orf\.at/radio/(?P<station>{0})/sendung/(?P<id>\d+)(?:/(?P<show>\w+))?".format(
            _STATION_RE
        ),
        r"https?://(?P<station>{0})\.orf\.at/player/(?P<date>\d{{8}})/(?P<id>\d+)".format(
            _STATION_RE
        ),
    )
        ...
        if "date" in m.groupdict():
            data = self._download_json(
                "https://audioapi.orf.at/{0}/json/4.0/broadcast/{1}/{2}?_o={3}.orf.at".format(
                    api_station, show_id, m.group("date"), station
                ),
                show_id,
            )
            show_id = data["id"]
...
class ORFRadioCollectionIE(ORFRadioBase):
    ...
    def _real_extract(self, url):
        coll_id, item_id = self._match_valid_url(url).group(
            "coll_id", "item_id"
        )
        data = self._download_json(
        ...
        def item_playlist(station, typed_item_id, item_type):
            if item_type == "upload":
                item_data = self._download_json(
                    "https://audioapi.orf.at/radiothek/api/2.0/upload/{0}?_o=sound.orf.at".format(
                        typed_item_id
                    ),
                    typed_item_id,
                )
                ...
                item_data = self._download_json(
                    "https://audioapi.orf.at/radiothek/api/2.0/episode/{0}?_o=sound.orf.at".format(
                        typed_item_id
                    ),
                    typed_item_id,
                )
            ...
            return merge_dicts(
                {"_type": "multi_video"},
                self.playlist_result(
                    self._entries(item_data, station, item_type),
                    typed_item_id,
                    txt_or_none(data.get("title")),
                    clean_html(data.get("subtitle")),
                ),
...
class ORFPodcastIE(ORFRadioBase):
    ...
    _STATION_RE = (
        "|".join(
            map(re.escape, (x[0] for x in ORFRadioBase.STATION_INFO.values()))
        )
        + "|tv"
    )
    ...
    def _real_extract(self, url):
        ...
        return merge_dicts(
            ...
            self._extract_podcast_upload(data),
            rev=True,
        )
...
class ORFIPTVBase(InfoExtractor):
    ...
    def _extract_video(self, video_id, webpage, fatal=False):
        ...
        for format_id, format_url in traverse_obj(
            rendition, ("redirect", T(dict.items), Ellipsis)
        ):
            if format_id == "rtmp":
                ff = f.copy()
                ff.update(
        ...
        return merge_dicts(
            {
                "id": video_id,
                "title": re.sub(
                    self._TITLE_STRIP_RE, "", self._og_search_title(webpage)
                ),
...
class ORFFM4StoryIE(ORFIPTVBase):
    ...
    def _real_extract(self, url):
        ...
        for idx, video_id in enumerate(
            re.findall(r'data-video(?:id)?="(\d+)"', webpage)
        ):
        ...
        for yt_id in re.findall(
            ...
            if YoutubeIE.suitable(yt_id):
                entries.append(
                    self.url_result(yt_id, ie="Youtube", video_id=yt_id)
                )
        ...
        return self.playlist_result(
            ...
            re.sub(
                ...
                "",
                self._og_search_title(webpage, default="") or None,
            ),
        )
...
class ORFONBase(InfoExtractor):
    ...
    def _call_api(self, video_id, **kwargs):
        encrypted_id = base64.b64encode(
            "{0}{1}".format(self._ENC_PFX, video_id).encode("utf-8")
        ).decode("ascii")
    ...
    def _parse_metadata(cls, api_json):
        return traverse_obj(
            api_json,
            {
                ...
                "duration": (
                    (
                        ("exact_duration", T(k_float_or_none)),
                ...
                if not self._yes_playlist(
                    ...
                    segments = [
                        next(
                            s
                            for s in segments
                            if txt_or_none(s["id"]) == segment_id
                        )
                    ]
...
class ORFONLiveIE(ORFONBase):
    ...
    _VALID_URL = (
        r"https?://on\.orf\.at/livestream/(?P<id>\d+)(?:/(?P<segment>\d+))?"
    )
    _TESTS = [
        {

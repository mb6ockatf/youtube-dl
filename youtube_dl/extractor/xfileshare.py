...
def aa_decode(aa_code):
    ...
    return "".join(chr_from_code(aa_char) for aa_char in aa_code.split(delim))
...
class XFileShareIE(InfoExtractor):
    _SITES = (
        ...
        (
            r"(?:vedbam\.xyz|vadbam.net)",
            "V?dB?m",
        ),  # vidbom.com redirects here, but no valid media pages listed
        ...
        (
            r"xvideosharing\.com",
            "XVideoSharing",
        ),  # just started showing 'maintenance mode'
    )
    ...
    def _real_extract(self, url):
        ...
        url = "https://%s/%s" % (
            host,
            (
                "embed-%s.html" % video_id
                if host in ("govid.me", "vidlo.us")
                else video_id
            ),
        )
        ...
        if self._search_regex(
            r">This server is in maintenance mode\.",
            container_div,
            "maint error",
            group=0,
            default=None,
        ...
        if not traverse_obj(result, "formats"):
            ...
            urls.update(
                traverse_obj(
                    sources,
                    (T(lambda s: self._parse_json(s, video_id)), Ellipsis),
                )
            )

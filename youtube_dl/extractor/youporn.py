        ...
        if not watchable:
            ...
            raise ExtractorError(
                (
                    ...
                    if msg
                    else "Video unavailable: no reason found"
                ),
                expected=True,
            )
        # internal ID ?
...
class YouPornListBase(InfoExtractor):
    ...
    def _entries(self, url, pl_id, html=None, page_num=None):
        ...
        PLAYLIST_SECTION_RE = r"""<div\s[^>]*\bclass\s*=\s*('|")(?:[\w$-]+\s+|\s)*?title-bar(?:\s+[\w$-]+|\s)*\1[^>]*>"""
        # contains video link
        VIDEO_URL_RE = r"""(?x)
        ...
        def retry_page(msg, tries_left, page_data):
            ...
            return next(yield_pages(page_data[0], page_num=page_data[2]), None)
        ...
        def yield_entries(html):
            for frag in re.split(PLAYLIST_SECTION_RE, html):
                ...
                if not (
                    t_text and re.search(self._PLAYLIST_TITLEBAR_RE, t_text)
                ):
                    continue
        ...
        for page_data in yield_pages(url, html=html, page_num=page_num):
            ...
            while tries_left > 0:
                ...
                for from_ in yield_entries(page_data[1]):
                    ...
                    if (
                        not first_url
                        and "no-result-paragarph1" in page_data[1]
                    ):
                        page_data = retry_page(
...
class YouPornCollectionIE(YouPornListBase):
    ...
    def _real_extract(self, url):
        ...
        playlist = super(YouPornCollectionIE, self)._real_extract(
            url, html=html
        )
        infos = re.sub(
            r"\s+",

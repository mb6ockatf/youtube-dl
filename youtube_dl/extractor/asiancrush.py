...
class AsianCrushBaseIE(InfoExtractor):
    ...
    def _call_api(self, host, endpoint, video_id, query, resource):
        return self._download_json(
            "https://api%s.%s/%s"
            % (self._API_SUFFIX.get(host, ""), host, endpoint),
            video_id,
    ...
    def _download_object_data(self, host, object_id, resource):
        return self._call_api(
            host, "search", object_id, {"id": object_id}, resource
        )[0]
    ...
    def _parse_video_data(self, video):
        ...
        meta_categories = (
            try_get(video, lambda x: x["meta"]["categories"], list) or []
        )
        categories = list(
            filter(None, [c.get("name") for c in meta_categories])
        )
...
class AsianCrushIE(AsianCrushBaseIE):
    _VALID_URL = (
        r"%s/video/(?:[^/]+/)?0+(?P<id>\d+)v\b"
        % AsianCrushBaseIE._VALID_URL_BASE
    )
    _TESTS = [
        {
...
class AsianCrushPlaylistIE(AsianCrushBaseIE):
    _VALID_URL = (
        r"%s/series/0+(?P<id>\d+)s\b" % AsianCrushBaseIE._VALID_URL_BASE
    )
    _TESTS = [
        {

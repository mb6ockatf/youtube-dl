...
class KonserthusetPlayIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        source = next(
            f for f in playlist if f.get("bitrates") or f.get("provider")
        )
        ...
        connection_url = player_config.get("rtmp", {}).get(
            "netConnectionUrl"
        ) or player_config.get("plugins", {}).get("bwcheck", {}).get(
            "netConnectionUrl"
        )
        if connection_url:
                ...
                f.update(
                    {
                        ...
                        "play_path": video_url,
                        "format_id": (
                            "rtmp-%s" % format_id if format_id else "rtmp"
                        ),
                        "ext": "flv",
                    }
                )
                    ...
                    f.update(
                        {
                            "url": fallback_url,
                            "format_id": (
                                "http-%s" % format_id if format_id else "http"
                            ),
                        }
                    )
                    formats.append(f)

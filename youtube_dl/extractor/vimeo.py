...
class VimeoIE(VimeoBaseInfoExtractor):
    ...
    def _extract_urls(url, webpage):
        ...
        for mobj in re.finditer(
            ...
            urls.append(
                VimeoIE._smuggle_referrer(unescapeHTML(mobj.group("url")), url)
            )
        PLAIN_EMBED_RE = (
...
class VimeoReviewIE(VimeoBaseInfoExtractor):
    ...
    _VALID_URL = (
        r"(?P<url>https://vimeo\.com/[^/]+/review/(?P<id>[^/]+)/[0-9a-f]{10})"
    )
    _TESTS = [
        {
...
class VimeoLikesIE(VimeoChannelIE):
    ...
    def _real_extract(self, url):
        ...
        return self._extract_videos(
            user_id, "https://vimeo.com/%s/likes" % user_id
        )

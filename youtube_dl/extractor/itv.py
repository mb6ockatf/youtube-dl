...
class ITVBaseIE(InfoExtractor):
    ...
    def __handle_request_webpage_error(
        self, err, video_id=None, errnote=None, fatal=True
    ):
        if errnote is False:
            return False
        if errnote is None:
        ...
        if fatal:
            raise ExtractorError(
                errmsg, sys.exc_info()[2], cause=err, video_id=video_id
            )
        else:
    ...
    def _download_webpage_handle(self, url, video_id, *args, **kwargs):
        ...
        ret = super(ITVBaseIE, self)._download_webpage_handle(
        ...
        if urlh.getcode() == 403:
            ...
            if (
                '"Request Originated Outside Of Allowed Geographic Region"'
                in webpage
            ):
...
class ITVIE(ITVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        formats = []
        for media_file in (
            traverse_obj(video_data, "MediaFiles", expected_type=list) or []
        ):
        ...
        subtitles = {}
        for sub in (
            traverse_obj(video_data, "Subtitles", expected_type=list) or []
        ):
        ...
        if not num_ep_title:
            num_ep_title = clean_html(
                get_element_by_attribute(
                    "data-testid", "episode-hero-description-strong", webpage
                )
            )
        ...
        def get_thumbnails():
            tns = []
            for w, x in (
                traverse_obj(video_data, ("imagePresets"), expected_type=dict)
                or {}
            ).items():
        ...
        return merge_dicts(
            {
                ...
                "episode_number": int_or_none(
                    (num_ep_title or "").split(".")[0]
                ),
                "channel": video_str("channel"),
                "categories": traverse_obj(
                    video_data, ("categories", "formatted"), expected_type=list
                ),
                "age_limit": {False: 16, True: 0}.get(
                    video_data.get("isChildrenCategory")
                ),
            },
            info,
        )
...
class ITVBTCCIE(ITVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        content = traverse_obj(
            content,
            ("body", "content", Ellipsis, "data"),
            expected_type=lambda x: (
                x
                if x.get("name") == "Brightcove"
                or x.get("type") == "Brightcove"
                else None
            ),
        )
            ...
            for video_id in re.findall(
                ...
                yield self.url_result(
                    smuggle_url(
                        self.BRIGHTCOVE_URL_TEMPLATE
                        % (
                            self.BRIGHTCOVE_ACCOUNT,
                            self.BRIGHTCOVE_PLAYER,
                            video_id,
                        ),
                        contraband,
                    ),

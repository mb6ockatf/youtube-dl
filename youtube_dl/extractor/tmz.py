    ...
    def _real_extract(self, url):
        ...
        if webpage:
            ...
            video = self._download_json(
                "https://www.tmz.com/_/video/%s" % tmz_video_id,
                video_id,
                fatal=False,
            )
            if video:
                ...
                if jwplatform_id:
                    ...
                    kaltura_entry_id = (
                        message.get("kaltura_entry_id") or video_id
                    )
                    kaltura_partner_id = (
                        message.get("kaltura_partner_id") or "591531"
                    )
                    info.update(
                        {
                            "url": "kaltura:%s:%s"
...
class TMZArticleIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?tmz\.com/\d{4}/\d{2}/\d{2}/(?P<id>[^/?#&]+)"
    )
    _TEST = {

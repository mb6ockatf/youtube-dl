    ...
    def _extract_from_legacy_playlist(self, playlist, playlist_id):
        ...
        for item in self._extract_items(playlist):
            ...
            description_el = playlist.find(
                "./{%s}summary" % self._EMP_PLAYLIST_NS
            )
            description = (
...
class BBCIE(BBCCoUkIE):
    ...
    IE_DESC = "BBC"
    _VALID_URL = (
        r"https?://(?:www\.)?bbc\.(?:com|co\.uk)/(?:[^/]+/)+(?P<id>[^/#?]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        if morph_payload:
            components = (
                try_get(morph_payload, lambda x: x["body"]["components"], list)
                or []
            )
            for component in components:
                ...
                if not identifiers or not isinstance(identifiers, dict):
                    continue
                programme_id = identifiers.get("vpid") or identifiers.get(
                    "playablePid"
                )
                if not programme_id:
                    continue
                title = lead_media.get("title") or self._og_search_title(
                    webpage
                )
                formats, subtitles = self._download_media_selector(
                    programme_id
        ...
        if preload_state:
            current_programme = (
                preload_state.get("programmes", {}).get("current") or {}
            )
        ...
        entries.extend(
            re.findall(r'setPlaylist\("(%s)"\)' % EMBED_URL, webpage)
        )
        if entries:
            return self.playlist_result(
...
class BBCCoUkIPlayerPlaylistBaseIE(InfoExtractor):
    _VALID_URL_TMPL = (
        r"https?://(?:www\.)?bbc\.co\.uk/iplayer/%%s/(?P<id>%s)"
        % BBCCoUkIE._ID_REGEX
    )

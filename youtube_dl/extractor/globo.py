    ...
    def _real_extract(self, url):
        ...
        for resource in video["resources"]:
            ...
            if not security_hash:
                ...
                if message:
                    raise ExtractorError(
                        "%s returned error: %s" % (self.IE_NAME, message),
                        expected=True,
                    )
                continue
            ...
            elif resource_id.endswith("manifest") or resource_url.endswith(
                "/manifest"
            ):
                formats.extend(
...
class GloboArticleIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if GloboIE.suitable(url)
            else super(GloboArticleIE, cls).suitable(url)
        )

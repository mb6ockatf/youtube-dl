...
class XuiteIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://vlog\.xuite\.net/(?:play|embed)/(?P<id>%s)" % _REGEX_BASE64
    )
    _TESTS = [
        {
            # Audio
    ...
    def _real_extract(self, url):
        ...
        media_info = self._parse_json(
            self._search_regex(
                r"var\s+mediaInfo\s*=\s*({.*});", webpage, "media info"
            ),
            video_id,
        )
            ...
            formats.append(
                {
                    ...
                    "format_id": format_id,
                    "height": (
                        int(format_id) if format_id.isnumeric() else None
                    ),
                }
            )
        ...
        return {
            ...
            "description": remove_end(
                media_info.get("metaDesc"), " (Xuite 影音)"
            ),
            ...
            "duration": float_or_none(
                media_info.get("MEDIA_DURATION"), 1000000
            ),

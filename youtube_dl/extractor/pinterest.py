...
class PinterestBaseIE(InfoExtractor):
    ...
    def _extract_video(self, data, extract_formats=True):
        ...
        title = (
            data.get("title") or data.get("grid_title") or video_id
        ).strip()
        ...
        if extract_formats:
            ...
            self._sort_formats(
                formats,
                field_preference=("height", "width", "tbr", "format_id"),
            )
        ...
        def _u(field):
            return try_get(
                data, lambda x: x["closeup_attribution"][field], compat_str
            )
...
class PinterestCollectionIE(PinterestBaseIE):
    ...
    def _real_extract(self, url):
        ...
        board = self._call_api(
            "Board", slug, {"slug": slug, "username": username}
        )["data"]
        board_id = board["id"]
        options = {
            "board_id": board_id,

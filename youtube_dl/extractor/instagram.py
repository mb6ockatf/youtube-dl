    ...
    def _real_extract(self, url):
        ...
        if media:
            ...
            comment_count = get_count(
                ("preview_comment", "to_comment", "to_parent_comment"),
                "comment",
            )
            ...
            if not video_url:
                edges = (
                    try_get(
                        media,
                        lambda x: x["edge_sidecar_to_children"]["edges"],
                        list,
                    )
                    or []
                )
                        ...
                        entries.append(
                            {
                                ...
                                "width": int_or_none(
                                    try_get(
                                        node,
                                        lambda x: x["dimensions"]["width"],
                                    )
                                ),
                                "height": int_or_none(
                                    try_get(
                                        node,
                                        lambda x: x["dimensions"]["height"],
                                    )
                                ),
    ...
    def _extract_graphql(self, data, url):
        ...
        for page_num in itertools.count(1):
            ...
            for edge in edges:
                ...
                thumbnail = node.get("thumbnail_src") or node.get(
                    "display_src"
                )

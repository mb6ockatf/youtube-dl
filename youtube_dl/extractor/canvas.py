...
class CanvasIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not data:
            ...
            if not data.get("title"):
                ...
                raise ExtractorError(
                    data.get("message") or code, expected=True
                )
        ...
        for target in data["targetUrls"]:
            format_url, format_type = url_or_none(
                target.get("url")
            ), str_or_none(target.get("type"))
        ...
        if isinstance(subtitle_urls, list):
            for subtitle in subtitle_urls:
                ...
                if subtitle_url and subtitle.get("type") == "CLOSED":
                    subtitles.setdefault("nl", []).append(
                        {"url": subtitle_url}
                    )
...
class CanvasEenIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "url": "https://mediazone.vrt.be/api/v1/%s/assets/%s"
            % (site_id, video_id),
...
class VrtNUIE(GigyaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return merge_dicts(
            ...
            {
                "_type": "url_transparent",
                "url": "https://mediazone.vrt.be/api/v1/vrtvideo/assets/%s"
                % video_id,
                "ie_key": CanvasIE.ie_key(),
                "id": video_id,
                "display_id": display_id,

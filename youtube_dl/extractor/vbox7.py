...
class Vbox7IE(InfoExtractor):
    ...
    def _parse_json(self, json_string, video_id, *args, **kwargs):
        if '"@context"' in json_string[:30]:
            ...
            transform_source = (
                args[0] if len(args) > 0 else kwargs.get("transform_source")
            )
    ...
    def _real_extract(self, url):
        ...
        if traverse_obj(response, "error"):
            raise ExtractorError(
                "%s said: %s" % (self.IE_NAME, response["error"]),
                expected=True,
            )

        src_url = (
            traverse_obj(response, ("options", "src", T(url_or_none))) or ""
        )
        ...
        if src_url:
            ...
            highest_res = (
                traverse_obj(video, ("highestRes", T(int))) or resolutions[0]
            )
            resolutions = (
                traverse_obj(
                    video,
                    ("resolutions", lambda _, r: highest_res >= int(r) > 0),
                )
                or resolutions
            )
            mp4_formats = traverse_obj(
                resolutions,
                (
                    Ellipsis,
                    T(
        ...
        info = (
            self._search_json_ld(
                webpage.replace('"/*@context"', '"@context"'),
                video_id,
                fatal=False,
            )
            if webpage
        ...
        if not info.get("title"):
            info["title"] = traverse_obj(
                response, ("options", "title", T(txt_or_none))
            ...
            traverse_obj(
                ...
                (
                    "options",
                    {
                        ...
                        "timestamp": (
                            "ago",
                            T(if_missing("timestamp")),
                            T(lambda t: int(round((now - t) / 60.0)) * 60),
                        ),
                        "duration": (
                            "duration",
                            T(if_missing("duration")),
                            T(float_or_none),
                        ),
                    },
                ),

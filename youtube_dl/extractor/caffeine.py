...
class CaffeineTVIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?caffeine\.tv/[^/]+/video/(?P<id>[0-9a-f-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        broadcast_info = (
            traverse_obj(json_data, ("broadcast_info", T(dict))) or {}
        )
        ...
        if ext == "m3u8":
            formats = self._extract_m3u8_formats(
                video_url, video_id, "mp4", entry_protocol="m3u8", fatal=False
            )
        else:

    ...
    def _real_extract(self, url):
        ...
        if video_id:
            ...
            info_page = self._download_webpage(
                "http://vk.com/video_ext.php?" + mobj.group("embed_query"),
                video_id,
            )
...
class VKWallPostIE(VKBaseIE):
    ...
    def _real_extract(self, url):
        ...
        description = clean_html(
            get_element_by_class("wall_post_text", webpage)
        )
            ...
            entries.append(
                {
                    ...
                    "title": (
                        "%s - %s" % (performer, title) if performer else title
                    ),
                    "thumbnails": (
                        [{"url": c_url} for c_url in a.cover_url.split(",")]
                        if a.cover_url
                        else None
                    ),
        ...
        for video in re.finditer(
            ...
            entries.append(
                self.url_result(
                    compat_urlparse.urljoin(url, video.group("url")),
                    VKIE.ie_key(),
                )
            )

...
class KalturaIE(InfoExtractor):
    ...
    def _extract_urls(webpage):
        ...
        for mobj in finditer:
            ...
            if service_mobj:
                url = smuggle_url(
                    url, {"service_url": service_mobj.group("id")}
                )
            urls.append(url)
        return urls

    def _kaltura_api_call(
        self, video_id, actions, service_url=None, *args, **kwargs
    ):
        params = actions[0]
        if len(actions) > 1:
    ...
    def _real_extract(self, url):
        ...
        if source_url:
            referrer = base64.b64encode(
                "://".join(compat_urlparse.urlparse(source_url)[:2]).encode(
                    "utf-8"
                )
            ).decode("utf-8")
        else:
        ...
        return {
            ...
            "uploader_id": (
                info.get("userId") if info.get("userId") != "None" else None
            ),

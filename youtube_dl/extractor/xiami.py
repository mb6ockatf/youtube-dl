...
class XiamiBaseIE(InfoExtractor):
    ...
    def _download_webpage_handle(self, *args, **kwargs):
        webpage = super(XiamiBaseIE, self)._download_webpage_handle(
            *args, **kwargs
        )
    ...
    def _extract_track(self, track, track_id=None):
        track_name = (
            track.get("songName") or track.get("name") or track["subName"]
        )
        artist = (
            track.get("artist")
            or track.get("artist_name")
            or track.get("singers")
        )
    ...
    def _extract_tracks(self, item_id, referer, typ=None):
        playlist = self._download_json(
            "%s/%s%s"
            % (self._API_BASE_URL, item_id, "/type/%s" % typ if typ else ""),
            item_id,
            headers={
                "Referer": referer,
            },

...
class IGNBaseIE(InfoExtractor):
    def _call_api(self, slug):
        return self._download_json(
            "http://apis.ign.com/{0}/v3/{0}s/slug/{1}".format(
                self._PAGE_TYPE, slug
            ),
            slug,
        )
    ...
    def _extract_video_info(self, video, fatal=True):
        ...
        thumbnails = traverse_obj(
            video,
            ("thumbnails", Ellipsis, {"url": "url"}),
            expected_type=url_or_none,
        )
        tags = traverse_obj(
            video,
            ("tags", Ellipsis, "displayName"),
...
class IGNVideoIE(IGNBaseIE):
    ...
    def _real_extract(self, url):
        ...
        new_url = urlh.geturl()
        ign_url = compat_parse_qs(compat_urlparse.urlparse(new_url).query).get(
            "url", [None]
        )[-1]
        if ign_url:

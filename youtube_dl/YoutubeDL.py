...
class YoutubeDL(object):
    ...
    def _bidi_workaround(self, message):
        ...
        res = "".join(
            self._output_channel.readline().decode("utf-8")
    ...
    def prepare_filename(self, info_dict):
        ...
        try:
            ...
            template_dict["autonumber"] = (
                self.params.get("autonumber_start", 1)
                - 1
                + self._num_downloads
            )
            if template_dict.get("resolution") is None:
                if template_dict.get("width") and template_dict.get("height"):
                    ...
                    template_dict["resolution"] = (
                        "%sp" % template_dict["height"]
                    )
                elif template_dict.get("width"):
                    template_dict["resolution"] = (
                        "%dx?" % template_dict["width"]
                    )
            ...
            template_dict = collections.defaultdict(
                lambda: self.params.get("outtmpl_na_placeholder", "NA"),
                template_dict,
            )
            ...
            FIELD_SIZE_COMPAT_RE = (
                r"(?<!%)%\((?P<field>autonumber|playlist_index)\)s"
            )
            ...
            if sys.version_info < (3, 0) and sys.platform == "win32":
                filename = encodeFilename(filename, True).decode(
                    preferredencoding()
                )
    ...
    def _load_cookies(self, data, **kwargs):
        ...
        for cookie in (
            compat_http_cookies_SimpleCookie(data).values() if data else []
        ):
    ...
    def process_ie_result(self, ie_result, download=True, extra_info={}):
        ...
        if result_type == "video":
            ...
            new_result = dict(
                (k, v) for k, v in extra_info.items() if k not in ie_result
            )
            if new_result:
            ...
            if isinstance(selector, list):
                ...
                def _merge(formats_info):
                    ...
                    return {
                        ...
                        "stretched_ratio": formats_info[0].get(
                            "stretched_ratio"
                        ),
    ...
    def _calc_headers(self, info_dict, load_cookies=False):
        if load_cookies:  # For --load-info-json
            ...
            self._load_cookies(
                traverse_obj(
                    info_dict, ("http_headers", "Cookie"), casesense=False
                ),
    ...
    def _fill_common_fields(self, info_dict, final=True):
        ...
        if final:
            for field in ("chapter", "season", "episode"):
                if info_dict.get(
                    "%s_number" % field
                ) is not None and not info_dict.get(field):
                    info_dict[field] = "%s %d" % (
                        field.capitalize(),
    ...
    def process_video_result(self, info_dict, download=True):
        ...
        if thumbnails:
            thumbnails.sort(
                key=lambda t: (
                    (
                        t.get("preference")
                        if t.get("preference") is not None
                        else -1
                    ),
        ...
        info_dict["http_headers"] = (
            self._remove_cookie_header(info_dict.get("http_headers") or {})
            or None
        )
        ...
        if not formats_to_download:
            raise ExtractorError(
                "requested format not available", expected=True
            )
    ...
    def __forced_printings(self, info_dict, filename, incomplete):
        ...
        if self.params.get("forceurl", False) and not incomplete:
            if info_dict.get("requested_formats") is not None:
                ...
                self.to_stdout(
                    info_dict["url"] + info_dict.get("play_path", "")
                )
    ...
    def process_info(self, info_dict):
        ...
        max_downloads = int_or_none(self.params.get("max_downloads")) or float(
            "inf"
        )
                ...
                if self.params.get("nooverwrites", False) and os.path.exists(
                    ...
                    if sub_info.get("data") is not None:
                        ...
                        try:
                            ...
                            with open(
                                encodeFilename(sub_filename), "wb"
                            ) as subfile:
        ...
        if not self.params.get("skip_download", False):
            try:
                ...
                def dl(name, info):
                    fd = checked_get_suitable_downloader(info, self.params)(
                        self, self.params
                    )
                ...
                if info_dict.get("requested_formats") is not None:
                    ...
                    def correct_ext(filename, ext=exts[1]):
                        ...
                        filename_wo_ext = (
                            f_name if f_real_ext in exts else filename
                        )
                        if ext is None:
            ...
            if success and filename != "-":
                ...
                try:
                    ...
                    self.report_error(
                        "postprocessing: %s" % error_to_compat_str(err)
                    )
                    return
    ...
    def sanitize_info(info_dict, remove_private_keys=False):
        ...
        if remove_private_keys:
            reject = lambda k, v: (
                v is None
                or k.startswith("__")
                or k in ("requested_formats", "requested_subtitles")
            )
        else:
        ...
        def filter_fn(obj):
            ...
            elif obj is None or any(
                isinstance(obj, c)
                for c in (compat_integer_types, (compat_str, float, bool))
            ):
                return obj
            else:
                return repr(obj)
    ...
    def print_debug_header(self):
        ...
        def python_implementation():
            ...
            if impl_name == "PyPy" and hasattr(sys, "pypy_version_info"):
                return (
                    impl_name + " version %d.%d.%d" % sys.pypy_version_info[:3]
                )
            return impl_name
        ...
        if self.params.get("call_home", False):
            ipaddr = (
                self.urlopen("https://yt-dl.org/ip").read().decode("utf-8")
            )
            writeln_debug("Public IP address: %s" % (ipaddr,))
            latest_version = (
                self.urlopen("https://yt-dl.org/latest/version")
                .read()
                .decode("utf-8")
            )

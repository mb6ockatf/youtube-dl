...
class _AVMClass(object):
    ...
    def register_methods(self, methods):
        ...
        self.method_idxs.update(
            dict((idx, name) for name, idx in methods.items())
        )
...
class SWFInterpreter(object):
    ...
    def extract_function(self, avm_class, func_name):
        ...
        def resfunc(args):
            ...
            registers = (
                [avm_class.variables] + list(args) + [None] * m.local_count
            )
            stack = []
            ...
            while True:
                ...
                if opcode == 9:  # label
                    ...
                    arg_count = u30()
                    args = list(
                        reversed([stack.pop() for _ in range(arg_count)])
                    )
                    obj = stack.pop()
                    ...
                    if obj == StringClass:
                        if mname == "String":
                            assert len(args) == 1
                            assert isinstance(
                                args[0], (int, compat_str, _Undefined)
                            )
                            if args[0] == undefined:
                                res = "undefined"
                            else:

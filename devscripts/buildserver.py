...
def win_OpenSCManager():
    ...
    if not res:
        raise Exception(
            "Opening service manager failed - "
...
def win_service_main(service_name, real_main, argc, argv_raw):
    try:
        ...
        if not h:
            raise OSError(
                "Handler registration failed: %s" % ctypes.FormatError()
            )
...
class GITBuilder(GITInfoBuilder):
    def build(self):
        try:
            ...
            subprocess.check_output(
                ["git", "checkout", self.rev], cwd=self.buildPath
            )
    ...
    def build(self):
        try:
            proc = subprocess.Popen(
                [
                    os.path.join(self.pythonPath, "python.exe"),
                    "setup.py",
                    "py2exe",
                ],
...
class DownloadBuilder(object):
    ...
    def build(self):
        ...
        self.handler.send_header(
            "Content-Length", str(os.stat(self.srcPath).st_size)
        )

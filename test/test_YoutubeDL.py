...
class TestFormatSelection(unittest.TestCase):
    ...
    def test_default_format_spec(self):
        ...
        ydl = YDL({})
        self.assertEqual(
            ydl._default_format_spec({}, download=False),
            "bestvideo+bestaudio/best",
        )
        self.assertEqual(
            ydl._default_format_spec({"is_live": True}),
...
class TestYoutubeDL(unittest.TestCase):
    ...
    def test_prepare_filename(self):
        ...
        self.assertEqual(
            fname(NA_TEST_OUTTMPL, na_placeholder="none"), "none-none-1234.mp4"
        )
        self.assertEqual(
            fname(NA_TEST_OUTTMPL, na_placeholder=""), "--1234.mp4"
        )
    ...
    def test_postprocessors(self):
        ...
        self.assertTrue(
            os.path.exists(filename), "%s doesn't exist" % filename
        )
        self.assertTrue(
            os.path.exists(audiofile), "%s doesn't exist" % audiofile
        )
        os.unlink(filename)
        os.unlink(audiofile)
    ...
    def test_match_filter(self):
        class FilterYDL(YDL):
            ...
            def _match_entry(self, info_dict, incomplete):
                res = super(FilterYDL, self)._match_entry(
                    info_dict, incomplete
                )
                if res is None:
    ...
    def test_urlopen_no_file_protocol(self):
        ...
        ydl = YDL()
        self.assertRaises(
            compat_urllib_error.URLError, ydl.urlopen, "file:///etc/passwd"
        )
...
class TestYoutubeDLCookies(unittest.TestCase):
    ...
    def assertSameCookies(self, c1, c2, msg=None):
        return self.assertEqual(
            *map(self.comparable_cookies, (c1, c2)), msg=msg
        )

    def assertSameCookieStrings(self, c1, c2, msg=None):
        return self.assertSameCookies(
            *map(
                lambda c: compat_http_cookies_SimpleCookie(c).values(),
                (c1, c2),
            ),
            msg=msg
        )
    ...
    def test_header_cookies(self):
        ...
        test_url, test_domain = (
            t % ("yt.dl",) for t in ("https://%s/test", ".%s")
        )
        ...
        def test(
            ...
            try:
                ...
                if not error_re:
                    raise
                assertRegexpMatches(
                    self, e.args[0], error_re.join((".*",) * 2)
                )

        test(
            "test=value; Domain=" + test_domain,
            [cookie("test", "value", domain=test_domain)],
        )
        test(
            "test=value",
            [cookie("test", "value")],
            error_re="Unscoped cookies are not allowed",
        )
        test(
            "cookie1=value1; Domain={0}; Path=/test; cookie2=value2; Domain={0}; Path=/".format(
                test_domain
            ),
            [
        ...
        test(
            "test=value; Domain={domain}; Path={path}; Secure; Expires={expires}".format(
                **cookie_kw
            ),
            [cookie("test", "value", **cookie_kw)],
        )
        test(
            'test="value; "; path=/test; domain=' + test_domain,
            [cookie("test", "value; ", domain=test_domain, path="/test")],
            round_trip='test="value\\073 "; Domain={0}; Path=/test'.format(
                test_domain
            ),
        )
        test(
            "name=; Domain=" + test_domain,
            [cookie("name", "", domain=test_domain)],
        ...
        test(
            "cookie1=value; Domain={0}; cookie2=value".format(test_domain),
            [],
            headers=True,
            error_re="Invalid syntax",
        )
        ...
        test(
            ...
            [],
            headers=True,
            error_re="Passing cookies as a header is a potential security risk",
        )
    ...
    def test_infojson_cookies(self):
        ...
        def test(initial_info, note):
            ...
            self.assertTrue(
                ...
                msg=failure_msg(
                    "No cookies set in cookiejar after initial process"
                ),
            )
            ydl.cookiejar.clear()
            ...
            self.assertTrue(
                ydl.cookiejar.get_cookies_for_url(TEST_URL),
                msg=failure_msg(
                    "No cookies set in cookiejar after final process"
                ),
            )
            ydl.cookiejar.clear()
            for key in ("processed", "loaded", "final"):
                ...
                self.assertIsNone(
                    ...
                    msg=failure_msg(
                        "Cookie header not removed in {0} result".format(key)
                    ),
                )
                self.assertSameCookieStrings(
                    traverse_obj(
                    ...
                    COOKIES,
                    msg=failure_msg(
                        "No cookies field found in {0} result".format(key)
                    ),
                )

...
TEST_COOKIE_VALUE = join_nonempty(
    "name", "value", delim="=", from_dict=TEST_COOKIE
)
...
def ifExternalFDAvailable(externalFD):
    return unittest.skipUnless(
        externalFD.available(), externalFD.get_basename() + " not found"
    )
    ...
    def download(self, params, ep):
        with subprocess.Popen(
            ...
            if not process.poll():
                ...
                self.assertEqual(
                    ydl.download(["http://127.0.0.1:%d/%s" % (self.port, ep)]),
                    0,
                )
                self.assertEqual(
...
class TestCurlFD(unittest.TestCase):
    def test_make_cmd(self):
        with FakeYDL() as ydl:
            ...
            self.assertIn(
                TEST_COOKIE_VALUE, downloader._make_cmd("test", TEST_INFO)
            )
...
class TestAria2cFD(unittest.TestCase):
    def test_make_cmd(self):
        with FakeYDL() as ydl:
            ...
            self.assertIn(
                "--load-cookies=%s" % downloader._cookies_tempfile, cmd
            )
...
def ifFFmpegFDAvailable(externalFD):
    ...
    return unittest.skipUnless(avail, externalFD.get_basename() + " not found")
        ...
        with FakeYDL() as ydl:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    "-y",
                    "-cookies",
                    TEST_COOKIE_VALUE + "; path=/; domain=.example.com;\r\n",
                    "-i",
                    "http://www.example.com/",
                    "-c",
                    "copy",

...
class FakeYDL(YoutubeDL):
    ...
    def trouble(self, *args, **kwargs):
        s = (
            args[0]
            if len(args) > 0
            else kwargs.get("message", "Missing message")
        )
        raise Exception(s)
...
def expect_value(self, got, expected, field):
    if isinstance(expected, compat_str) and expected.startswith("re:"):
    ...
    elif isinstance(expected, compat_str) and expected.startswith(
        "startswith:"
    ):
        ...
        self.assertTrue(
            ...
            "field %s (value: %r) should start with %r"
            % (field, got, start_str),
        )
        ...
        self.assertTrue(
            contains_str in got,
            "field %s (value: %r) should contain %r"
            % (field, got, contains_str),
        )
        ...
        self.assertTrue(
            fn(got),
            "Expected field %s to meet condition %s, but value %r failed "
            % (field, suite, got),
        )
        ...
        self.assertEqual(
            expected,
            got,
            "Invalid value for field %s, expected %r, got %r"
            % (field, expected, got),
        )
...
def expect_info_dict(self, got_dict, expected_dict):
    ...
    if got_dict.get("_type") not in ("playlist", "multi_video"):
        for key in ("id", "url", "title", "ext"):
            self.assertTrue(
                got_dict.get(key), "Missing mandatory field %s" % key
            )

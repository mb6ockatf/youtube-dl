...
class TestCompat(unittest.TestCase):
    ...
    def test_compat_urllib_parse_unquote(self):
        ...
        self.assertEqual(
            compat_urllib_parse_unquote("%7e/abc+def"), "~/abc+def"
        )
        ...
        self.assertEqual(compat_urllib_parse_unquote("%2f"), "/")
        self.assertEqual(
            compat_urllib_parse_unquote("%E6%B4%A5%E6%B3%A2"), "津波"
        )
        self.assertEqual(
            compat_urllib_parse_unquote(
...
%<a href="https://ar.wikipedia.org/wiki/تسونامي">%a""",
    ...
    def test_compat_urllib_parse_unquote_plus(self):
        self.assertEqual(
            compat_urllib_parse_unquote_plus("abc%20def"), "abc def"
        )
        self.assertEqual(
            compat_urllib_parse_unquote_plus("%7e/abc+def"), "~/abc def"
        )

    def test_compat_urllib_parse_urlencode(self):
        self.assertEqual(
            compat_urllib_parse_urlencode({"abc": "def"}), "abc=def"
        )
        self.assertEqual(
            compat_urllib_parse_urlencode({"abc": b"def"}), "abc=def"
        )
        self.assertEqual(
            compat_urllib_parse_urlencode({b"abc": "def"}), "abc=def"
        )
        self.assertEqual(
            compat_urllib_parse_urlencode({b"abc": b"def"}), "abc=def"
        )
        self.assertEqual(
            compat_urllib_parse_urlencode([("abc", "def")]), "abc=def"
        )
...
<smil xmlns="http://www.w3.org/2001/SMIL20/Language"></smil>"""
    ...
    def test_compat_urllib_request_Request(self):
        ...
        self.assertEqual(PUTrequest("http://127.0.0.1").get_method(), "PUT")

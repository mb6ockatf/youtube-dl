...
class TestUtil(unittest.TestCase):
    ...
    def test_sanitized_Request(self):
        self.assertFalse(
            sanitized_Request("http://foo.bar").has_header("Authorization")
        )
        self.assertFalse(
            sanitized_Request("http://:foo.bar").has_header("Authorization")
        )
        self.assertEqual(
            sanitized_Request("http://@foo.bar").get_header("Authorization"),
            "Basic Og==",
        )
        self.assertEqual(
            sanitized_Request("http://:pass@foo.bar").get_header(
                "Authorization"
            ),
            "Basic OnBhc3M=",
        )
        self.assertEqual(
            sanitized_Request("http://user:@foo.bar").get_header(
                "Authorization"
            ),
            "Basic dXNlcjo=",
        )
        self.assertEqual(
            sanitized_Request("http://user:pass@foo.bar").get_header(
                "Authorization"
            ),
            "Basic dXNlcjpwYXNz",
        )
    ...
    def test_expand_path(self):
        ...
        self.assertEqual(
            expand_path(env("YOUTUBE_DL_EXPATH_PATH")), "expanded"
        )
    ...
    def assertUnsafeExtension(self, ext=None):
        ...
        def my_exit(self_, exc_type, exc_val, exc_tb):
            ...
            if did_raise and assert_raises.ext is not None:
                self.assertEqual(
                    assert_raises.ext,
                    assert_raises.exception.extension,
                    "Unsafe extension  not as unexpected",
                )
            return did_raise
    ...
    def test_date_from_str(self):
        ...
        self.assertEqual(
            date_from_str("now+365day"), date_from_str("now+1year")
        )
        self.assertEqual(
    ...
    def test_multipart_encode(self):
        ...
        self.assertEqual(
            multipart_encode(
                {"欄位".encode("utf-8"): "值".encode("utf-8")},
                boundary="AAAAAA",
            )[0],

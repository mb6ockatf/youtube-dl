...
class HTTPTestRequestHandler(compat_http_server.BaseHTTPRequestHandler):
    ...
    def super(self, meth_name, *args, **kwargs):
        ...
        try:
            ...
            fn = lambda s, m, *a, **k: getattr(
                compat_http_server.BaseHTTPRequestHandler, m
            )(s, *a, **k)
    ...
    def _test_url(self, path, host="127.0.0.1", scheme="http", port=None):
        return "{0}://{1}:{2}/{3}".format(
            scheme,
            host,
            port if port is not None else http_server_port(self.server),
            path,
        )
...
class TestHTTP(unittest.TestCase):
    ...
    _check_cert = sys.version_info >= (3, 2) or (
        sys.version_info[0] == 2 and sys.version_info[1:] >= (7, 19)
    )
    ...
    def setUp(self):
        ...
        self.http_server_thread = threading.Thread(
            target=self.http_httpd.serve_forever
        )
        ...
        try:
            ...
            class ThreadingHTTPServer(
                ThreadingMixIn, compat_http_server.HTTPServer
            ):
                pass
        ...
        self.https_server_thread = threading.Thread(
            target=self.https_httpd.serve_forever
        )
    ...
    def _test_url(self, path, host="127.0.0.1", scheme="http", port=None):
        return "{0}://{1}:{2}/{3}".format(
            ...
            (
                port
                if port is not None
                else self.https_port if scheme == "https" else self.http_port
            ),
            path,
        )

    @unittest.skipUnless(
        _check_cert, "No support for certificate check in SSL"
    )
    def test_nocheckcertificate(self):
        with FakeYDL({"logger": FakeLogger()}) as ydl:
            with self.assertRaises(compat_urllib_error.URLError):
                ydl.urlopen(
                    sanitized_Request(
                        self._test_url("headers", scheme="https")
                    )
                )

        with FakeYDL(
            {"logger": FakeLogger(), "nocheckcertificate": True}
        ) as ydl:
            r = ydl.urlopen(
                sanitized_Request(self._test_url("headers", scheme="https"))
            )
    ...
    def test_unicode_path_redirection(self):
        with FakeYDL() as ydl:
            r = ydl.urlopen(
                sanitized_Request(self._test_url("302-non-ascii-redirect"))
            )
    ...
    def test_redirect(self):
        with FakeYDL() as ydl:
            ...
            def do_req(redirect_status, method, check_no_content=False):
                ...
                return res.read().decode("utf-8"), res.headers.get(
                    "method", ""
                )
    ...
    @expectedFailureIf(
        sys.platform.startswith("java") and sys.version_info < (2, 7, 2)
    )
    def test_content_type(self):
        ...
        with FakeYDL({"nocheckcertificate": True}) as ydl:
            ...
            r = sanitized_Request(
                self._test_url("headers", scheme="https"),
                data=urlencode_postdata({"test": "test"}),
            )
    ...
    def test_cookiejar(self):
        with FakeYDL() as ydl:
            ...
            data = (
                ydl.urlopen(sanitized_Request(self._test_url("headers")))
                .read()
                .decode("utf-8")
            )
    ...
    def test_gzip_trailing_garbage(self):
        ...
        with FakeYDL() as ydl:
            data = (
                ydl.urlopen(
                    sanitized_Request(self._test_url("trailing_garbage"))
                )
                .read()
                .decode("utf-8")
            )
    ...
    def __test_compression(self, encoding):
        with FakeYDL() as ydl:
            ...
            self.assertEqual(
                res.read(), b'<html><video src="/vid.mp4" /></html>'
            )
    ...
    def test_remove_dot_segments(self):
        with FakeYDL() as ydl:
            res = ydl.urlopen(
                sanitized_Request(self._test_url("a/b/./../../headers"))
            )
            self.assertEqual(
                compat_urllib_parse.urlparse(res.geturl()).path, "/headers"
            )

            res = ydl.urlopen(
                sanitized_Request(self._test_url("redirect_dotsegments"))
            )
            self.assertEqual(
                compat_urllib_parse.urlparse(res.geturl()).path, "/headers"
            )
...
def _build_proxy_handler(name):
    class HTTPTestRequestHandler(compat_http_server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(
                "{0}: {1}".format(self.proxy_name, self.path).encode("utf-8")
            )
...
class TestProxy(unittest.TestCase):
    def setUp(self):
        ...
        self.geo_proxy_thread = threading.Thread(
            target=self.geo_proxy.serve_forever
        )
    ...
    def _test_proxy(self, host="127.0.0.1", port=None):
        return "{0}:{1}".format(host, port if port is not None else self.port)
